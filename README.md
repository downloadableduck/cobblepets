...
PetsMod is a Minecraft mod developed by downloadableduck and the pets team. This mod brings custom, client-sided pets into your game that will follow you around and interact with you. As of 0.8.0, PetsMod supports user-made addons, which allow new custom pets to exist. 
...
petsmod_version=0.8.1-26.1.1-alpha
...
Additionally, make sure to depend on PetsMod `0.8.0` or greater in your `fabric.mod.json`. 
...
```java
    ...
    private static void refreshPetNames(CallbackInfo ci) {
        if (Utils.checkName(CUSTOM_ENTITY_VALUE, customEntity, CUSTOM_CONFIG.customEntityName)) {
            customEntity.setName(CUSTOM_CONFIG.customEntityName);
        }
    }
```
`Utils.checkName`:
